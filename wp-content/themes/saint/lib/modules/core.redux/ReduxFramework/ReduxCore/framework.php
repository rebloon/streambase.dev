...
//                $comparison = $field['required'][1];
                ...
                if( isset( $section['fields'] ) ) {
                    foreach( $section['fields'] as $fieldk => $field ) {
                        ...
                        if( isset( $field['title'] ) && isset( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "group" && $field['type'] !== "section" ) {
						    ...
						    /* AC REMOVE Add Desc to left side */
								if (!empty($field['desc'])) {
	              	$field['description'] = $field['desc'];
                }
								                    
                $th .= ( isset( $field['description'] ) && $field['type'] != "info"  && $field['type'] !== "section" && $field['type'] != "group" && !empty( $field['description'] ) ) ? '<div class="description field-desc">' . $field['description'] . '</div>' : '';						    
        ...
        public function _options_page_html() {
            ...
            // AC - Documentation link
            echo "<div class='online-documenation'><a href='http://alleycatthemes.com/themes/saint/documentation/' target='_blank'>Online Documentation</a></div>";
            ...
            // AC - Add Demo Data header
            echo '<li id="ac_demo_data_default_section_group_li" class="redux-group-tab-link-li">';

            echo '<a href="javascript:void(0);" id="ac_demo_data_default_section_group_li_a" class="redux-group-tab-link-a" data-rel="ac_demo_data_default"><i class="el-icon-cogs icon-large"></i><span class="group_title">' . __( 'Demo Data', $this->args['domain'] ) . '</span></a>';
            echo '</li>';
 
            echo '<li class="divide">&nbsp;</li>';
            // AC - Add Demo Data header / End                        
            ...
            foreach( $this->sections as $k => $section ) {
                echo '<div id="' . $k . '_section_group' . '" class="redux-group-tab ac-'.$k.'_section_group">';
                if ( !empty( $section['sections'] ) ) {
                	//$tabs = "";
            ...
            if( $this->args['show_import_export'] === true ) {
               ...
               echo '<div id="ac_demo_data_default_section_group' . '" class="redux-group-tab">';
                echo '<h3>' . __( 'Import Demo Data Page', $this->args['domain'] ) . '</h3>';
                ?>                 
										...
										<?php
											...
											if ($missing_demo_files) {
												...
												<div class='demo-data-missing-files'>
													...
													<p><a class='button button-primary' href='http://alleycatthemes.com/support/forum/theme/saint/' target='_blank'>Contact Support and let them know</a></p>
												</div>
												<?php								
											}
											else {
...
/* AC REMOVE Desc from right side
	                    if (!empty($field['desc'])) {
	                    	$field['description'] = $field['desc'];
	                    }
